...
The worker retains the process-local component-fixture MP4 proof endpoints with explicit 720p, 1080p, 1440p, and 4K choices. Project drafts use a separate durable Convex queue: one exact composition and narration version, a full or selected range, a 640×360 maximum, H.264/AAC, progress, cancellation, bounded lease recovery, and a storage-backed download. Shared frame planning keeps browser preview and draft component-local frames aligned. Project jobs are serialized, frame capture and encoding do not overlap, H.264 uses the `veryfast` preset, and the real worker lowers its OS scheduling priority. Pi execution belongs to MED-128.
