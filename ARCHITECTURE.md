...
MED-153 closes M2 with a real-route golden proof rather than a second
orchestration layer. The proof retains project, proposal, immutable composition,
render, and model telemetry as separate authoritative records. Provider
transport failures must become failed creator-visible messages; an assistant
event with an error stop reason cannot be persisted as an empty successful
response. The measured run and the intentionally deferred collaboration,
research extraction, general editing-agent, and production-rendering boundaries
are recorded in `docs/M2_GOLDEN_RUN.md`.
