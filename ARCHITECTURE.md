...
Verified candidate previews should run in a sandboxed frame isolated from the main application. Final and draft rendering occurs in the worker environment. The first render path produces a resolution- and quality-pinned MP4 at 720p, 1080p, 1440p, or 4K from one exact component version. Creator-facing quality presets use H.264 CRF 18, 22, or 28, with CRF 18 as the default for crisp text and programmatic graphics; composition, audio, and broader codec options expand in later milestones. Higher resolutions remain explicit creator choices because they consume more worker time and memory. Local rendering favors machine responsiveness over throughput: one job and one Remotion frame worker run at a time, frame rendering and encoding do not overlap, H.264 uses a fast preset, and the worker lowers its OS scheduling priority where supported. MED-133 will replace the process-local queue with durable bounded jobs without weakening these resource controls.
